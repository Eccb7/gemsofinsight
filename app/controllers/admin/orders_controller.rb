class Admin::OrdersController < AdminController
  ...
  def index
    ...
    @not_fulfilled_orders = Order.where(fulfilled: false).offset(@page * @per_page).limit(@per_page)
    @not_fulfilled_total_pages = (Order.where(fulfilled: false).count / @per_page.to_f).ceil

    @fulfilled_orders = Order.where(fulfilled: true).offset(@page * @per_page).limit(@per_page)
    @fulfilled_total_pages = (Order.where(fulfilled: true).count / @per_page.to_f).ceil
  end
